...
pok TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the pok control plane API.

## Install (workspace)

This package is internal to the `newne` monorepo. Add it as a
...
```ts
import { pok } from "@saas/sdk";

const client = new pok({
  baseUrl: "https://api.l.com",
  auth: { kind: "bearer", token: process.env["NEWNE_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `pokError`. Branch on
...
| `pokError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `pokError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new pok({ fetch })`.
