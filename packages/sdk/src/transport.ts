// Base HTTP transport for the pok SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `pokError`
...
export interface ClientOptions {
  /**
   * Base URL of the pok api-edge worker, e.g.
   * `https://api.l.com`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `pok` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "pok SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "pok SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
