...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new pok({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => pokError]> = [
  ...
  it("forward-compat: unknown error code → base pokError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(pokError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as pokError).code).toBe("future_quota_exceeded");
    }
  });
