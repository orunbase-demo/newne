...
describe("resolveConfigDir", () => {
  it("respects NEWNE_CONFIG_DIR override", () => {
    const prev = process.env["NEWNE_CONFIG_DIR"];
    process.env["NEWNE_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["NEWNE_CONFIG_DIR"];
      else process.env["NEWNE_CONFIG_DIR"] = prev;
    }
  });
});
