// `newne whoami` — read token + cached context, validate via SDK.
...
export interface WhoamiInput {
  ...
  /** SDK factory override for tests. Defaults to constructing `pok`. */
  readonly sdkFactory?: (baseUrl: string, token: string) => pok;
}
