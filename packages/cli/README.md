...
`newne` — first-class TypeScript CLI for the pok control
...
This package is internal to the `newne` monorepo and is not
...
```
newne login    [--api-url=URL] [--token=BEARER]
newne logout
newne whoami
```
...
```
newne org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
newne project create <name> [--idempotency-key=KEY]
newne env create <project-id> <name> [--idempotency-key=KEY]
newne api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
newne webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
newne usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
newne billing summary
newne audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`newne org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `newne login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/newne/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/newne/config.json` (mode 0644, not a secret). Override
both via `NEWNE_CONFIG_DIR` (used by tests).
